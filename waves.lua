...
if _G.WAVES_SCRIPT_LOADED then
    return
end
_G.WAVES_SCRIPT_LOADED = true
...
local success, err = pcall(function()
    ...
    -- Debugging: Notify that the script has started
    alert("WAVES Script Initialized.")
    ...
    safespotPart.Position = Vector3.new(0, -1000, 0) -- Place it out of view

    -- Define _autoget function
    local function _autoget()
        task.spawn(function()
            while task.wait(3) and getgenv().misc do
                ...
                for _, v in ipairs(map:GetDescendants()) do
                    if string.match(string.lower(v.Name), 'lost') and v:IsA('BasePart') then
                        v.CFrame = lplr.Character:WaitForChild('HumanoidRootPart').CFrame
                    elseif string.match(string.lower(v.Name), 'rescue') and v:FindFirstChild('Contact') then
                        v.Contact.CFrame = lplr.Character:WaitForChild('HumanoidRootPart').CFrame
                    end
                end
            end
        end)
    ...
    local function initializeAutofarmForMap(map)
        if getgenv().gettingbuttons then
            ...
            alert("Autofarm started for new map.")
        end
    end
                ...
                task.spawn(function()
                    while task.wait(5) and getgenv().safespot do
                        ...
                        if hrpPos and (hrpPos - safespotPart.Position).Magnitude > 250 then
                            ...
                            lplr.Character.HumanoidRootPart.CFrame = CFrame.new(safespotPart.Position + Vector3.new(0, 5, 0))
                        end
                    end
                end)
            end
    ...
    alert('WAVES V2.52 loaded successfully.')
